...
const ChatApp = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      socket.emit('leave room', ROOM_ID); // Leave the room on unmount
  ...
  const sendMessage = () => {
    ...
    // Check if the message is empty and no file is attached
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    textarea.style.height = `${Math.min(textarea.scrollHeight, 96)}px`; // Limit the height to 96px (4 lines)
  };
          ...
          {messages.map((message) => (
            ...
            >
              ...
              <div className='flex items-end max-w-xs md:max-w-md lg:max-w-lg'>
                ...
                <div className={`p-3 rounded-lg ${message.sender === 'user' ? 'self-end border-blue-500 border-2' : 'self-start border-green-500 border-2'} max-w-[100%] whitespace-pre-wrap overflow-hidden`}>
                  ...
                  {message.file && (
                    <div className='mt-2 flex items-center'>
                      ...
                      <a
                        ...
                        className='ml-2 text-blue-600 flex items-center'
                      >
        ...
        <div className='p-4 bg-white shadow-md flex flex-col items-center'>
          ...
          <div className='relative flex items-center w-full border border-gray-300 rounded-lg p-2 pr-4'>
            <textarea
              ...
              rows={1}
              style={{ height: '24px', maxHeight: '96px', overflowY: 'auto' }}
            />
            <div className='flex items-center space-x-2'>
              <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className='text-blue-500 hover:text-blue-800 focus:outline-none'>
                <FiSmile className='w-6 h-6' />
              </button>
              <label className='cursor-pointer text-blue-500 hover:text-blue-800'>
                <FaPaperclip className='w-6 h-6' />
                <input type='file' className='hidden' onChange={handleFileChange} />
              </label>
              <button onClick={sendMessage} className='text-blue-500 hover:text-blue-800 focus:outline-none'>
